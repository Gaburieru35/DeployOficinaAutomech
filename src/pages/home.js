...
const Home = () => {
  ...
  async function getIndicadoresAvaliacao(){
    let url = `https://4.227.162.137:443/Avaliacao/indicadores`
  ...
  async function getIndicadores(){
    let url = `https://4.227.162.137:443/Reparo/Indicadores`
    const response = await fetch(url, { method: 'GET' })
    var data = await response.json();
    console.log(data)
  ...
  async function indicadorTrimestralClientes(){
    let url = `https://4.227.162.137:443/Cliente/indicador`
    const response = await fetch(url, { method: 'GET' })
